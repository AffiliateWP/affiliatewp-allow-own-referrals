...
class AffiliateWP_Activation {
    ...
    public function __construct( $plugin_path, $plugin_file ) {
        ...
        if ( isset( $plugins[$this->plugin_path . '/' . $this->plugin_file]['Name'] ) ) {
            ...
            $this->plugin_name = __( 'This plugin', 'affiliatewp-allow-own-referrals' );
        }
    ...
    public function missing_affiliatewp_notice() {
        ...
        if ( $this->has_affiliatewp ) {
			...
			echo '<div class="error"><p>' . sprintf( __( '%1$s requires %2$s. Please activate it to continue.', 'affiliatewp-allow-own-referrals' ), $this->plugin_name, '<a href="https://affiliatewp.com/" target="_new">AffiliateWP</a>' ) . '</p></div>';

        } else {

        	/* translators: 1: Plugin name, 2: AffiliateWP */
        	echo '<div class="error"><p>' . sprintf( __( '%1$s requires %2$s. Please install it to continue.', 'affiliatewp-allow-own-referrals' ), $this->plugin_name, '<a href="https://affiliatewp.com/" target="_new">AffiliateWP</a>' ) . '</p></div>';
